...
const ProductosSecos = () => {
  ...
  const loadItemsFromStorage = async () => {
    try {
      ...
      if (user && user.usuario) {
        const itemsData = await AsyncStorage.getItem(`productosSecos-${user.usuario}`); // Obtener los artículos de la clave correspondiente al usuario
        if (itemsData) {
  ...
  const saveItemsToStorage = async (newItems) => {
    try {
      const user = JSON.parse(await AsyncStorage.getItem('currentUser')); // Obtener el usuario actual del AsyncStorage
      if (user && user.usuario) {
        await AsyncStorage.setItem(`productosSecos-${user.usuario}`, JSON.stringify(newItems)); // Guardar los artículos en la clave correspondiente al usuario
      } else {
        console.log('Error: currentUser no tiene usuario.');
      }
    } catch (error) {
  ...
  return (
    <ImageBackground source={require('../../../assets/FondoRosa.jpg')} style={styles.container}>
      <Text style={styles.title}>Productos Secos</Text>
