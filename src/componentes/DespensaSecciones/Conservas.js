...
const Conservas = () => {
  ...
  const loadItemsFromStorage = async () => {
    try {
      ...
      if (user && user.usuario) {
        const itemsData = await AsyncStorage.getItem(`congelados-${user.usuario}`);
        if (itemsData) {
  ...
  const saveItemsToStorage = async (newItems) => {
    try {
      const user = JSON.parse(await AsyncStorage.getItem('currentUser'));
      if (user && user.usuario) {
        await AsyncStorage.setItem(`congelados-${user.usuario}`, JSON.stringify(newItems));
      } else {
        console.log('Error: currentUser no tiene usuario.');
      }
    } catch (error) {
  ...
  return (
    <ImageBackground source={require('../../../assets/FondoRosa.jpg')} style={styles.container}>
      <Text style={styles.title}>Conservas</Text>
